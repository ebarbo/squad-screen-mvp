...
| Companions | [`operational-runbook.md`](./operational-runbook.md) · [`submission-checklist.md`](./submission-checklist.md) · [`limitations.md`](./limitations.md) · [`customer-and-pilot.md`](./customer-and-pilot.md) |
...
**Repository state (VERIFIED, 2026-09-22).** `main` was at `6754824` when this was written and carries EWE-60 through EWE-64: the scaffold and published command set, the server-only configuration loader, the shared Zod contracts and examples, CI, the fixture and source packet, the Token Factory adapter, and the evidence pipeline. It does **not** yet carry synthesis (EWE-65), scenarios (EWE-66), the APIs (EWE-67) or the UI (EWE-68–EWE-70).

`main` has been moving every few minutes. Rows below are timestamped by what was on `main` when they were written; re-check this paragraph and the ledger in §3 before the packet is assembled.

So the commands in [`operational-runbook.md`](./operational-runbook.md) are real and the fixture and evidence layers exist, but **every on-screen description below is still SPECIFIED** — derived from the technical contract's UI contract and the deliverables of EWE-68, EWE-69 and EWE-70, not from anything anyone has run.
...
Generation latency is **PENDING** — no generation has been timed. Measure it three times in rehearsal 1 and take the slowest, then apply:
...
| 0:55 | Point at the constraint on the card carrying `player_actions`. | Planned minutes visible against the staff-supplied 45-minute limit. The limit is in the packet (EWE-62), assembled from dated staff records (EWE-64) and validated in code (EWE-65). |
...
| 1:20 | **Pause 3 s.** Let the room read it. | Exact excerpt or record, source name, URL or record ID, observed / published / retrieved dates, and `data_mode`. **SPECIFIED** — technical contract, `EvidenceItem`. |
| 1:40 | *Optional, only if the drawer surfaces it:* point at the shared-origin pair. | Two outlets carrying the same agency copy, collapsed to one source. The pair is in the packet (EWE-62) and EWE-64 collapses it; whether it renders is EWE-69. |
...
> **Do not present the public URLs as live links, and do not invite anyone to click one.** EWE-62 records that the illustrative public URLs use the reserved `example.invalid` domain rather than reproducing third-party content. The outlet, dates, excerpt and snapshot structure are what to show. A reviewer who clicks a dead link and then re-reads your provenance claims has been handed a reason to doubt all of them.

**Both halves of that third sentence are load-bearing and defensible.** EWE-64 collapses syndicated copies only when the origin *and* the excerpt match, because a club site publishes many distinct statements under one origin. And where independent sources disagree, both are marked disputed with resolution left null — the commit is explicit that no averaging step exists to blend them. The packet contains exactly such an unresolved disagreement about one player's availability. If the drawer surfaces it on your chosen card, that is the strongest ten seconds in this beat; confirm which card in rehearsal 1.
...
| 2:12 | Point at the evidence snapshot ID. | Same `evidence_snapshot_id` as before the flip. EWE-64 makes it a **content hash over the evidence set**, so an identical ID means the facts are byte-identical, not merely labelled unchanged. |
...
**If a reviewer presses on that**, the answer is concrete: the ID is a content hash over the evidence set, and the contexts are deep-frozen before they leave the module, so the scenario layer has to copy because it cannot write. A matching ID is evidence, not an assertion.
...
**Everything in this beat is PENDING [EWE-73](https://linear.app/ewerton-barbosa/issue/EWE-73/eval-run-the-controlled-comparison-and-publish-observed-results).** No number exists. `npm run eval` exits 1 by design until EWE-71 lands, and `model-comparison.md` has not been written anywhere. **Pick the variant on the morning of the demo, from what that file actually says.**
...
**If asked about tokens or cost:** the comparison model is a reasoning model that spends substantially more completion tokens for an equivalent answer, so telemetry reads the provider's usage fields and never estimates from response length. And the API exposes no pricing, usage or billing endpoint — prices have to come from the Nebius console, so without them the estimated cost is reported as `null` with that reason rather than guessed. Say that if it comes up; do not volunteer a comparison that has not been run.
...
>
> We could not run it. ⟨State the actual reason — harness not finished, or no credentials.⟩ So the table says *Pending* — not a number. The command that produces it is in the repository and it takes minutes once the blocker clears.
>
...
**Naming the model.** EWE-63 defaults both IDs and records them as confirmed reachable in the event account: **`Qwen/Qwen3-235B-A22B-Instruct-2507`** primary and **`openai/gpt-oss-120b`** for the comparison. They are identifiers, not secrets, so naming them is safe. The primary is the non-thinking Instruct variant on purpose — Apache-2.0, instruction-tuned, and it returns the answer directly with no reasoning preamble to strip; that is a good ten-second answer if a technical reviewer asks why.

Even so: **state what ran, not what is configured.** Read the model off the telemetry of the run you just performed. Configuration is an intention; telemetry is a fact, and they diverge exactly when it matters most.
...
Rows marked *commit record* were read from the landing commit's message and a directory listing rather than from the files themselves. That is a weaker basis than reading the code, and it is labelled so nobody treats it as stronger.
...
| 3 | 45-minute limit; supported alternative option | **Landed** — `data/demo` and `data/sources` on `main` (*commit record* + listing); EWE-64 assembles squad state from dated records | Reading the fixture files, or EWE-72 exercising them |
...
| 6 | Evidence excerpt, dates, record shown at beat 3 | **Packet and pipeline landed** (EWE-62, EWE-64). Whether it renders is **PENDING EWE-69** | The drawer working on the chosen card |
| 7 | Shared-origin collapse visible at beat 3 | **Implemented** — EWE-64 collapses on matching origin *and* excerpt (*commit record*). Visibility **PENDING EWE-69** | Rehearsal 1, on the chosen card |
| 8 | Which branch beat 4 takes | **PENDING EWE-66 / EWE-72** | Rehearsal 1 |
| 9 | Snapshot ID unchanged after the flip | **Enforced** — EWE-64 makes the ID a content hash and deep-freezes contexts, with a test asserting a nested write throws (*commit record*) | EWE-72 exercising it through a real re-run |
| 10 | Every comparison number at beat 5 | **PENDING EWE-73** | EWE-71, then `npm run eval` — which exits 1 today |
| 11 | Generation and re-evaluation latency | **PENDING** — nothing has been timed | Rehearsal 1, timed three times |
| 12 | Customer status and pilot measures at beat 6 | **Sourced** to `customer-and-pilot.md`, being landed on `main` by its owner; currently `b5093cc` on `feat/ewe-74-customer-and-pilot`. EWE-74 itself is not Done (its §9) | EWE-74 closing |
...
| 14 | Model IDs named at beat 7 | **Defaulted and recorded as confirmed reachable** in the event account (EWE-63). Safe to name | Still read the model off telemetry: state what ran, not what is configured |
...
| 19 | "No confidence percentages anywhere" (beat 2) | **Landed, not independently read.** EWE-61 records no confidence field in any schema, a strict synthesis schema, a prose-percentage check and passing tests. EWE-64 adds that derived metrics are computed in code, never asked of a model | Reading `src/domain/contracts.ts`, or EWE-72 confirming none renders |
| 20 | Public source URLs shown at beat 3 | **Illustrative.** EWE-62 records the reserved `example.invalid` domain rather than reproducing third-party content | Nothing — a property of the packet. Handle it by not presenting them as live links |
...
| --- | --- | --- | --- |
| 1 | `NEBIUS_API_KEY` — the only mandatory provider variable | Ewerton | Before rehearsal 1 |
