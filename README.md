...
npm run check:env            # preflight: is the provider usable?
...
| `npm run demo:intelligence` | Headless generate → re-evaluate → diff | **exits 1 until EWE-67** |
...
| `SQUAD_SCREEN_COMPARISON_MODEL_ID` | EWE-73 | Second approved model for the controlled comparison |
...
trace. Cost is left `null` with a stated reason rather than guessed.
...
2. **Immutability under the overlay.** A scenario applies availability overrides to a *copy* of the
   base `MatchContext`. The base run and its `evidence_snapshot_id` are frozen once created.
   Re-evaluation re-runs synthesis only — no re-extraction, no search — and returns
   added / revised / withdrawn / unchanged against prior recommendation IDs. Withdrawn advice stays
   inspectable.
